...
<body class="flex flex-col h-screen overflow-hidden font-sans antialiased text-stone-800">
    ...
    <div class="flex flex-1 overflow-hidden relative">
        ...
        <main class="flex-1 overflow-y-auto p-4 sm:p-8">
            @if(session('success'))
                <div class="mb-6 p-4 bg-emerald-50 border border-emerald-100 text-emerald-700 rounded-2xl text-sm font-semibold flex justify-between items-center shadow-xs" id="flash-banner">
                    <span>🎉 {{ session('success') }}</span>
                    <button onclick="document.getElementById('flash-banner').remove()" class="text-emerald-400 hover:text-emerald-600 cursor-pointer font-bold">✕</button>
                </div>
            @endif
